...
Navigate to the directory where you wish to place the source code. In this example, we'll use `/usr/local/` as the base directory.
    ...
    cd /usr/local/
    git clone https://github.com/YOUR_USERNAME/billetsys billetsys
    cd billetsys
    git remote add upstream https://github.com/billetsys/billetsys
    git fetch --all
...
One is from your GitHub account repository, and the other is from the `billetsys` repository
