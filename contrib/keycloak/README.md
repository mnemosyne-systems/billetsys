...
> **Known limitation (follow-up for #50):** this migration makes Keycloak the
> required identity provider — all `/api/*` endpoints expect a Bearer JWT and
> the legacy cookie session no longer authenticates API calls. The original
> goal of keeping database login as the default unless Keycloak is installed
> is therefore not yet met. Making Keycloak opt-in (database login by default)
> is tracked as follow-up work; this PR intentionally lands the Keycloak path
> first so both modes can be validated separately.
...
The defaults match the local setup above, so no extra configuration is needed
locally. For any other Keycloak server, copy the template and adjust it —
Vite loads it automatically on `npm run dev` / build:
...
VITE_KEYCLOAK_REALM=billetsys
VITE_KEYCLOAK_CLIENT_ID=billetsys-frontend
```
